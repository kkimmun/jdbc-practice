...
public class SpeciesView {
	...
	private void save() {
		...
		System.out.println("종 추가하는 기능입니다.");
		...
		if(list.isEmpty()) {
			System.out.println("조회결과가 존재하지 않습니다.");
				...
				for(SpeciesDto sd : list) {
					...
					System.out.println("종분류 : " + sd.getSpeicesClass());
					System.out.println();
				}
		}
	...
	private void update() {
		...
		// SPECIES_ID값을 입력받아서
		// SPEICES_NAME과 SPECIES_CLASS값을 수정하는 기능
		...
		String speicesName = sc.nextLine();
		System.out.print("수정하실 종 분류를 입력하세요 > ");
		String speciesClass = sc.nextLine();
		
		int result = 
		speciesController.updatae(new SpeciesDto(speciesId,
												 speicesName,
												 speciesClass));
	...
	private void findByKeyword() {
		System.out.println("이름으로 검색 서비스입니다.");
